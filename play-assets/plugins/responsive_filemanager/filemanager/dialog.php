...
if(isset($_GET["img"])) {
	if ($_GET["img"] == "Y") {
		$_SESSION["img"] = "Y";
		$img_get = "img=Y&";
		$upload_dir = IMG_DIR_FILEMANAGER;
		$relative_path = IMG_PATH_FILEMANAGER;
	}
	else {
		$_SESSION["img"] = "N";
		$img_get = "img=N&";
		$upload_dir = UPLOAD_DIR_FILEMANAGER;
		$relative_path = UPLOAD_PATH_FILEMANAGER;
	}
}
else {
	$img_get = "";
...
<body>
	...
	if(isset($_GET["img"]))
		if ($_GET["img"] == "Y") 
			$cur_url = str_replace(array('&img=Y','&filter='.$filter,'&sort_by='.$sort_by,'&descending='.intval($descending)),array(''),$base_url.$_SERVER['REQUEST_URI']);
		else 
			$cur_url = str_replace(array('&img=N','&filter='.$filter,'&sort_by='.$sort_by,'&descending='.intval($descending)),array(''),$base_url.$_SERVER['REQUEST_URI']);
	else 
		$cur_url = str_replace(array('&filter='.$filter,'&sort_by='.$sort_by,'&descending='.intval($descending)),array(''),$base_url.$_SERVER['REQUEST_URI']);
	?>
	<input type="hidden" id="current_url" value="<?php echo $cur_url; //edited by playcms?>" />
...
<!-- header div end -->
	...
	<div class="row-fluid">
	<?php
	$link="dialog.php?".$img_get.$get_params;
	?>
	<ul class="breadcrumb">
	...
	<?php } ?>
	<li class="pull-right"><a id="refresh" class="btn-small" href="dialog.php?<?php echo $img_get.$get_params.$subdir."&".uniqid() ?>"><i class="icon-refresh"></i></a></li>
	...
	<div class="span12">
		...
		foreach ($files as $file_array) {
			...
			?>
				...
				<?php } ?>
				<a class="folder-link" href="dialog.php?<?php echo $img_get.$get_params.rawurlencode($src)."&".uniqid() ?>">
			...
			<?php }else{ ?>
					...
					<div class="box">
					<h4 class="<?php if($ellipsis_title_after_first_row){ echo "ellipsis"; } ?>"><a class="folder-link" data-file="<?php echo $file ?>" href="dialog.php?<?php echo $img_get.$get_params.rawurlencode($src)."&".uniqid() ?>"><?php echo $file;?></a></h4>
					</div>
