 ...
 */
	...
	class User_model extends CI_Model {
		...
		public function all_user_datatables($dt) {
			...
			$this->db->select($columns . ', u.level_id, ul.id AS levels_id, ul.name AS level_name')
							//->from($dt['table'])
							->join('user_levels as ul', '`u`.`level_id` = `ul`.`id`');
							//->get($dt['table']);
			//$sql2 = $sql;
			...
			//$rowCount = $sql2->get($dt['table'])->num_rows();
			//$sql2->free_result();
			...
			if(isset($dt['search']['value']) and $dt['search']['value'] != "") {
				$search = $dt["search"]["value"];
				//$where = "";
				for ($i=0; $i < $count_c ; $i++) {
					if($i==0)
						$this->db->like($columnd[$i], $search);
					else
						$this->db->or_like($columnd[$i], $search);
			...
			if(isset($dt['order'])){
				...
				$this->db->order_by($columnd[$dt['order'][0]['column']], $dt['order'][0]['dir']);
			}
			
			// LIMIT ..., ...
			if(isset($dt['start']) and isset($dt['length'])) {
				$start  = $dt['start'];
				$length = $dt['length'];
			...
			//echo 'value search : "'.$search.'"<br>';
			//echo $this->db->get_compiled_select($dt['table']);
			$list = $this->db->get($dt['table']);
			// Konversi Ke JSON
			$option['draw']            = $dt['draw'];
			$option['recordsTotal']    = $list->num_rows();//$rowCount;
			$option['recordsFiltered'] = $list->num_rows();//$rowCount;
			$option['data']            = array();
